<template>
  <CalcLayout>
    <template #header>
      <div class="mb-4">
        <h1 class="page-title">{{ t('calc.kgToYards.icon') }} {{ t('calc.kgToYards.title') }}</h1>
        <p class="page-subtitle">{{ t('calc.kgToYards.desc') }}</p>
      </div>
    </template>
    ...
    <template #results>
      <div v-if="result" class="section-card">
        ...
        <div class="result-box text-center mb-3">
          ...
          <div class="result-label">{{ t('res.yardsResult') }}</div>
        </div>
        <div class="grid grid-cols-2 gap-2">
          <div class="result-card">
            <div class="result-card-value">{{ result.meters }} m</div>
            <div class="result-card-label">{{ t('res.metersResult') }}</div>
          </div>
          <div class="result-card">
        ...
        <div class="formula-box">
          Step 1: Meters = (kg × 100,000) / (GSM × Width_cm)<br>
          Step 2: Yards = Meters × 1.09361
        </div>
      </div>
